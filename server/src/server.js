...
function render() {
  ...
  f.innerHTML = FILTERS.map(function (x) {
    return '<button class="btn ghost' + (filter === x[0] ? ' on' : '') + '" aria-pressed="' + (filter === x[0]) + '" onclick="setFilter(\\'' + x[0] + '\\')">' + x[1] + '</button>';
  }).join('');
  var rows = shown();
  ...
  document.getElementById('list').innerHTML = rows.map(function (l) {
    ...
    return '<div class="card">' + head +
      ...
      '<label for="f-name">نام</label><input id="f-name" autocomplete="off" value="' + esc(l.name) + '">' +
      '<label>امتیاز</label><div class="starpick" id="f-stars">' +
      [1, 2, 3, 4, 5].map(function (i) {
        return '<button type="button" aria-label="امتیاز ' + faDigits(i) + '" class="' + (l.rating >= i ? 'on' : '') + '" onclick="pick(' + i + ')">★</button>';
      }).join('') + '</div>' +
      ...
      '<label for="f-followup" style="display:block;margin-top:10px">برنامه پیگیری</label><input id="f-followup" autocomplete="off" value="' + esc(l.followup) + '">' +
      '<label for="f-note">یادداشت</label><textarea id="f-note" rows="2">' + esc(l.note) + '</textarea>' +
      '<div id="save-err" class="err" role="alert" style="display:none">ذخیره نشد؛ اینترنت را بررسی کنید و دوباره تلاش کنید</div>' +
      '<div class="row"><button class="btn" id="save-btn" onclick="save(\\'' + l.id + '\\')">ذخیره</button>' +
...
function save(id) {
  ...
  var errBox = document.getElementById('save-err');
  btn.disabled = true;
  ...
  fetch(BASE + '/admin/api/leads/' + id, {
    ...
    if (!r.ok) throw 0;
    return r.json();
  }).then(function () { editing = null; load(); }).catch(function () {
    btn.disabled = false;
